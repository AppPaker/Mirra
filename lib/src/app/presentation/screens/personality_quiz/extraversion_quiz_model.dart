...
class ExtraversionQuizManager {
  final FirebaseFirestore firestore = FirebaseFirestore.instance;
  final String userId;

  List<ExtraversionQuestion> questions = [];
  List<String> fetchedQuestionIds = [];

  ExtraversionQuizManager({required this.userId}) {
    if (kDebugMode) {
      print("User ID in ExtraversionQuizManager: $userId");
    }
  }
  ...
  Future<void> initializeQuestions() async {
    for (int i = 0; i < 4; i++) {
      ExtraversionQuestion newQuestion = await getRandomQuestion();
  ...
  Future<void> saveAnswer(int index) async {
    ...
    DocumentReference answerRef = firestore
        .collection('users')
        .doc(userId)
        .collection('extraversion_answers')
  ...
  Future<ExtraversionQuestion> getRandomQuestion() async {
    // Get a random document from the 'agreeable_questions' collection
    final response = await http.get(
      Uri.parse(
          'https://firestore.googleapis.com/v1/projects/mirr00/databases/(default)/documents/Questions_raw/extraversion/extraversion_questions'),
    );
    ...
    if (response.statusCode == 200) {
      ...
      ExtraversionQuestion question = ExtraversionQuestion(
