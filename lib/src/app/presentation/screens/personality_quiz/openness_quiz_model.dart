...
class OpennessQuizManager {
  final FirebaseFirestore firestore = FirebaseFirestore.instance;
  final String userId;

  List<OpennessQuestion> questions = [];
  List<String> fetchedQuestionIds = [];

  OpennessQuizManager({required this.userId}) {
    if (kDebugMode) {
  ...
  Future<void> initializeQuestions() async {
    for (int i = 0; i < 4; i++) {
      OpennessQuestion newQuestion = await getRandomQuestion();
  ...
  Future<OpennessQuestion> getRandomQuestion() async {
    // Get a random document from the 'agreeable_questions' collection
    final response = await http.get(
      Uri.parse(
          'https://firestore.googleapis.com/v1/projects/mirr00/databases/(default)/documents/Questions_raw/openness/openness_questions'),
    );
    ...
    if (response.statusCode == 200) {
      ...
      OpennessQuestion question = OpennessQuestion(
