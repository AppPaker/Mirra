...
class AgreeableQuizManager {
  final FirebaseFirestore firestore = FirebaseFirestore.instance;
  final String userId;

  List<AgreeableQuestion> questions = [];
  List<String> fetchedQuestionIds = [];

  AgreeableQuizManager({required this.userId}) {
    if (kDebugMode) {
      print("User ID in AgreeableQuizManager: $userId");
    }
  }
  ...
  Future<void> initializeQuestions() async {
    for (int i = 0; i < 4; i++) {
      AgreeableQuestion newQuestion = await getRandomQuestion();
  ...
  Future<AgreeableQuestion> getRandomQuestion() async {
    // Get a random document from the 'agreeable_questions' collection
    final response = await http.get(
      Uri.parse(
          'https://firestore.googleapis.com/v1/projects/mirr00/databases/(default)/documents/Questions_raw/agreeable/agreeable_questions'),
    );
    ...
    if (response.statusCode == 200) {
      ...
      AgreeableQuestion question = AgreeableQuestion(
