...
class ConscientiousnessQuestion {
  final String id;
...
class ConscientiousnessQuizManager {
  final FirebaseFirestore firestore = FirebaseFirestore.instance;
  final String userId;

  List<ConscientiousnessQuestion> questions = [];
  List<String> fetchedQuestionIds = [];

  ConscientiousnessQuizManager({required this.userId}) {
    if (kDebugMode) {
      print("User ID in OpennessQuizManager: $userId");
    }
  }
  ...
  Future<void> initializeQuestions() async {
    for (int i = 0; i < 4; i++) {
      ConscientiousnessQuestion newQuestion = await getRandomQuestion();
  ...
  Future<void> saveAnswer(int index) async {
    ...
    DocumentReference answerRef = firestore
        .collection('users')
        .doc(userId)
        .collection('conscientiousness_answers')
  ...
  Future<ConscientiousnessQuestion> getRandomQuestion() async {
    // Get a random document from the 'agreeable_questions' collection
    final response = await http.get(
      Uri.parse(
          'https://firestore.googleapis.com/v1/projects/mirr00/databases/(default)/documents/Questions_raw/conscientiousness/conscientiousness_questions'),
    );
    ...
    if (response.statusCode == 200) {
      ...
      ConscientiousnessQuestion question = ConscientiousnessQuestion(
